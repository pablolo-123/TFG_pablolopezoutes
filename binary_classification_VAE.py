...
class VAEClassifier(nn.Module):
    def __init__(self, num_classes_binary=1, fingerprint_dim=1489, latent_dim=256):
        ...
        # GIN layers
        nn1 = Sequential(Linear(78, 256), ReLU(), Linear(256, 512))
        self.conv1 = GINConv(nn1)
        self.bn1 = nn.BatchNorm1d(512)

        nn2 = Sequential(Linear(512, 512), ReLU(), Linear(512, 512))
        self.conv2 = GINConv(nn2)
        self.bn2 = nn.BatchNorm1d(512)

        nn3 = Sequential(Linear(512, 512), ReLU(), Linear(512, 512))
        self.conv3 = GINConv(nn3)
        self.bn3 = nn.BatchNorm1d(512)

        self.fc_1 = Linear(512, self.gnn_dim)
        ...
        # Final classifier
        self.fc_binary = nn.Sequential(
            nn.Linear(self.gnn_dim + latent_dim, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
...
def train_fn(model, optimizer, train_loader, val_loader, test_loader, epochs, beta=0.0):  ## add validation argument
    ...
    for epoch in range(epochs):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            loss_vae = vae_loss_function(recon_x, fp_tensor, mu, logvar)
...
if __name__ == '__main__':
    ...
    print("\nResultados guardados en kfold_results.csv")
