...
class MultiTaskModel(nn.Module):
    def __init__(self, num_classes_multi, num_classes_binary):
        ...
        # Gate fusion layers
        self.g_proj = nn.Linear(256, 256)
        self.fp_proj = nn.Linear(256, 256)
    ...
    def forward(self, data):
        ...
        # Gate fusion
        gate = torch.sigmoid(self.g_proj(x) + self.fp_proj(fpn_out))
        fusion = gate * x + (1 - gate) * fpn_out

        # 二分类任务的预测
        y_binary = self.fc_binary(fusion)
...
if __name__ == '__main__':
    ...
    df.to_csv("kfold_results_GineConcat.csv", index=False)
    print("\nResultados guardados en kfold_results_GineConcat.csv")
